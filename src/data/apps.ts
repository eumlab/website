// Ported verbatim from the legacy original/mainweb/models.py.
// Field names and list ordering are preserved exactly so the templates that
// iterate them produce identical markup.
...
export interface App {
  ...
  reviews?: string[];
  storeurl?: string;
  kw?: string;    // SEO keywords (from sitemap.py), used in <head>
  mdesc?: string; // SEO meta description (from sitemap.py), used in <head>
}

export const applist: Record<string, App> = {
...
// Press links (models.py adds these after the dict literal).
...
export const indexApps: App[] = [
  applist.pm, applist.instuner, applist.gm, applist.bt, applist.dl, applist.st,
  applist.dawhd, applist.ut, applist.iuke, applist.ukehero, applist.uke101,
  applist.ukechords, applist.uketube, applist.chordtunes,
];

export const indexAppsMobile: App[] = [
  applist.pm, applist.instuner, applist.gm, applist.bt, applist.dl, applist.ut,
  applist.iuke, applist.ukehero, applist.uke101, applist.ukechords, applist.uketube,
  applist.chordtunes, applist.st, applist.daw,
];
...
export const ukeApps: App[] = [
  applist.ut, applist.iuke, applist.ukehero, applist.uke101,
  applist.ukechords, applist.uketube, applist.chordtunes,
];

// --- User reviews (verbatim from models.py) ----------------------------------
applist.ut.reviews = [
...
// storeurl computed for every app, exactly as models.py does.
for (const key of Object.keys(applist)) {
  const item = applist[key];
  item.storeurl = 'http://itunes.apple.com/app/id' + item.id + '?ls=1&mt=8&at=11lt4f&ct=eumwebsite';
}

// SEO kw/mdesc (sitemap.py mutates applist with these in production; used in <head>).
for (const [key, v] of Object.entries(seo as Record<string, { kw: string; mdesc: string }>)) {
  if (applist[key]) {
    applist[key].kw = v.kw;
    applist[key].mdesc = v.mdesc;
  }
}

...
jQuery.fn.threesixty = function(options){
	...
	$(function() {
		...
		var progressBg = $("<div></div>").css({display:'none', width:parent.width()-200, height:10, backgroundColor:"black", position:"absolute","bottom":60,left:100 }).addClass("progressBg");
		var progressBar = $("<div></div>").css({display:'none',width:0, height:10, backgroundColor:"white", position:"absolute","bottom":60,left:100 }).data("progress",0).addClass("progressBar");
		var overlay;
		try {
		 overlay = $("<div></div>").css({display:'none',cursor:"wait", width:pic.width(), background:"RGBA(0,0,0,0.7)", height:pic.height(),  position:"absolute","top":0,left:0 }).addClass("overlay");
		}
		catch (e)
		{
		 overlay = $("<div></div>").css({display:'none',cursor:"wait", width:pic.width(), height:pic.height(), backgroundColor:"black", filter:"alpha(opacity=70)",  position:"absolute","top":0,left:0 }).addClass("overlay");
		}
		...
		$.each(options.images, function(index, record) {
			var o =$("<img>").attr("src",record).load(function() {
				...
				{	loaded=true;
                    ...
                    pic.parent().css("visibility","visible");
				}
			});
			cache.push(o); 
		});
